...
class Parser {
    ...
    public function compileFile(
        ...
        if (flock($fp, LOCK_EX)) {
            ...
            // read the file
            $this->templateInfo['code'] = $code = (filesize($templateFilepath) > 0 ? fread($fp, filesize($templateFilepath)) : '');
    ...
    protected function varReplace($html, $loopLevel = NULL, $escape = TRUE, $echo = FALSE) {
        ...
        if (preg_match_all('/(\$[a-z_A-Z][^\s\(\)|:]*)/', $html, $matches)) {
            ...
            for ($i = 0; $i < count($matches[1]); $i ++) {
                ...
                $rep = preg_replace('/[\.\s]++(?=(?:(?:[^"]*+"){2})*+[^"]*+$)(?=(?:(?:[^\']*+\'){2})*+[^\']*+$)(?=(?:[^()]*+\([^()]*+\))*+[^()]*+$)(\${0,1}[a-zA-Z_0-9]+)/', '["$1"]', $rep);
            ...
            if (!$init) {
                // translate _()
                if (preg_match('/^\$translate->_s?\(.+?\)$/s', $html)) {
                    $echo = true;
                    $escape = false;
                }
    ...
    protected function modifierReplace($html) {
        ...
        if (strpos($html,'|') !== false && substr($html,strpos($html,'|')+1,1) != "|") {
            preg_match('/([\$a-z_A-Z0-9\(\),\[\]"->]+)\|([\$a-z_A-Z0-9\(\):,\s\w\[\]"->]+)/ui', $html,$result);
            $function_params = $result[1];
            $explode = preg_split('/:(?=(?:[^\'"]|[\'"][^\'"]*[\'"])*$)/', $result[2]);
            $function = $explode[0];
