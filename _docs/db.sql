...
CREATE TABLE `m_user` (`user_id` binary(16) NOT NULL, `user_login` varchar(255) NOT NULL, `user_password` binary(20) NOT NULL, `user_session_id` char(40) DEFAULT NULL, `user_role_id` binary(16) DEFAULT NULL, `create_date` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, `update_date` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, `delete_date` datetime DEFAULT NULL, PRIMARY KEY (`user_id`), KEY `user_role_id` (`user_role_id`), CONSTRAINT `m_user_ibfk_2` FOREIGN KEY (`user_role_id`) REFERENCES `m_user_role` (`role_id`) ON DELETE SET NULL ON UPDATE SET NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

SET @m_user_role__uuid = UUID();
INSERT INTO `m_user_role` (`role_id`, `role_name`) VALUES (UUID_TO_BIN(@m_user_role__uuid), 'Administrator');
INSERT INTO `m_user_acl` (`acl_id`, `acl_module`, `acl_action`, `acl_type`, `role_id`, `user_id`) VALUES (UUID_TO_BIN(UUID()), 'main', NULL, 'ALLOW', UUID_TO_BIN(@m_user_role__uuid), NULL);

SET @m_user__uuid = UUID();
INSERT INTO `m_user` (`user_id`, `user_login`, `user_password`, `user_session_id`, `user_role_id`, `create_date`) VALUES (UUID_TO_BIN(@m_user__uuid), 'admin', UNHEX(SHA1('admin')), NULL, UUID_TO_BIN(@m_user_role__uuid), NOW());
